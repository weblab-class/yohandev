...
pub fn render(world: &mut World) {
    for (entity, shape) in &mut world.query::<Without<&Sprite, &Transform>>() {
        match shape {
    ...
    for (entity, (shape, transform)) in &mut world.query::<(&Sprite, &Transform)>() {
        // Translate local sprites
        let Vec2 { x: dx, y: dy } = transform.translation;
        match shape {
            Sprite::Rect(Vec2 { x, y }, Extent2 { w, h }) => unsafe {
                iter_sprites_rect(entity.id(), *x + dx, *y + dy, *w, *h);
            },
            Sprite::Circle(Vec2 { x, y }, r) => unsafe {
                iter_sprites_circle(entity.id(), *x + dx, *y + dy, *r);
            },
        }
    }
}